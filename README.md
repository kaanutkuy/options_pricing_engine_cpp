...
## Running the Pricer
After cloning the repo, type and run "make" in the CLI of the directory to build the Makefile, and then type and run "./main" to execute the main file. Follow the directions using the command line interface. It will prompt you to create an option by specifying the type of option, and the parameter inputs of the option, and then it will ask for a name to store the option object in memory. After you create it, you can retrieve the option's price, and depending on the type of it, the Greeks as well.
